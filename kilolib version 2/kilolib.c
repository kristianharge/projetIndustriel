...
direction view_rx_channel;          //the ADC channel we are viewing to get direction
uint8_t distances[3];
/*fin ISIR*/
...
static inline void estimate_direction(){
    ...
    distances[view_rx_channel - 1] = estimate_distance(&rx_dist);

    for(i = 0; i < 3; ++i)                      //0 is ADC1, 1 is ADC2 and 2 is ADC3
        if(distances[i] > max){
            max = distances[i];
            rx_direction = i;
        }

    if(view_rx_channel < 3)                   //the admux we are choosing to consider
        view_rx_channel += 1;
    else
        view_rx_channel = 1;
}
...
ISR(ANALOG_COMP_vect) {
    ...
    if(rx_leadingbit) {       // Start bit received.
        ...
        if (rx_leadingbyte) {
            ...
            //adc_trigger_low_gain(); //generic code in ADC1
            /*ISIR*/
            adc_trigger_low_gain(view_rx_channel);
        }
    } else {
        // Stray bit received
